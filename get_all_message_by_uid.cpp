...
int main() {
	...
	while (FCGI_Accept() >= 0) {
		...
		if(session->checkSession() == false){
			...
			snprintf(query_buf,sizeof(query_buf),"select b.send_id,a.username,b.message, b.send_time from users a INNER JOIN (select * from p2p_messages where rece_id = %d ) b on a.user_id = b.send_id",atoi(user_id.c_str()));
